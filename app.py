...
from flask import Flask,render_template,escape, request,url_for,redirect,flash
...
class User(db.Model,UserMixin): # 表名将会是 user（自动生成，小写处理）如果你想自己指定表名，可以定义 __tablename__ 属性
...
def edit(movie_id):
    ...
    if request.method == 'POST': # 处理编辑表单的提交请求
        ...
        if not title or not year or len(year) !=4 or len(year) >60:
            flash('Invalid input.')
